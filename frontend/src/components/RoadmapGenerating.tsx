...
const TOTAL_MS = STEPS.reduce((s, st) => s + st.ms, 0);   // ~7700 ms

interface Props {
  targetProfession?: string;
  isLoadingDone: boolean;
  onDoneShown: () => void;  // вызывается когда анимация завершена + API вернул результат
}

export const RoadmapGenerating = ({ targetProfession, isLoadingDone, onDoneShown }: Props) => {
  // stepProgress[i] = 0..100 для каждого шага
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      // Если API уже вернул ответ и анимация дошла до >= 85% — ускоряем к финишу
      const shouldFinish = doneRef.current && totalElapsed / TOTAL_MS >= 0.85;

      const boostFactor = shouldFinish ? 3 : 1;

      if (currentStep < STEPS.length) {
        const step = STEPS[currentStep];
        elapsed[currentStep] = Math.min(
          elapsed[currentStep] + INTERVAL * boostFactor,
          step.ms,
        );
        const stepPct = Math.round((elapsed[currentStep] / step.ms) * 100);

        setStepProgress((prev) => {
          const next = [...prev];
          next[currentStep] = stepPct;
          return next;
        });

        totalElapsed = elapsed.reduce((s, e) => s + e, 0);
        const ov = Math.min(Math.round((totalElapsed / TOTAL_MS) * 100), 99);
        setOverall(ov);

        // Переходим к следующему шагу когда текущий дошёл до 100%
        if (stepPct >= 100 && currentStep < STEPS.length - 1) {
          currentStep++;
        ...
        if (stepPct >= 100 && currentStep === STEPS.length - 1) {
          ...
          if (doneRef.current) {
            // API уже ответил — показываем 100% и переходим
            setOverall(100);
            setDone(true);
            setTimeout(() => onDoneShown(), 600);
          }
          // иначе ждём API (см. useEffect ниже)
        }
      }
    }, INTERVAL);
  ...
  useEffect(() => {
    ...
    // Если анимация уже завершена — сразу показываем 100%
    if (animDone.current) {
      setOverall(100);
      setDone(true);
      setTimeout(() => onDoneShown(), 600);
    }
    // иначе анимация ускорится сама (см. boostFactor выше)
  }, [isLoadingDone, onDoneShown]);
  ...
  return (
    <div
      style={{
        ...
        padding: "20px",
        transition: done ? "opacity 0.5s" : undefined,
        opacity: done ? 0 : 1,
      }}
    >
      {/* Ambient glow */}
      <div style={{
        ...
        background: "radial-gradient(circle, rgba(192,98,62,0.12) 0%, transparent 65%)",
      }} />
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{
            ...
            <div style={{
              height: "100%",
              width: `${overall}%`,
              background: "linear-gradient(90deg, #c0623e, #e8855a)",
              borderRadius: 3,
              transition: "width 0.25s ease",
              boxShadow: "0 0 12px rgba(192,98,62,0.6)",
            }} />
          </div>
        </div>
              ...
              <div
                ...
                style={{
                  ...
                  background: isActive
                    ...
                    : "transparent",
                  border: `1px solid ${isActive ? step.color + "44" : isCompleted ? "rgba(255,255,255,0.07)" : "transparent"}`,
                  transition: "all 0.3s",
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: isActive ? 10 : 0 }}>
                  ...
                  <span style={{
                    ...
                    color: isCompleted ? "rgba(255,255,255,0.25)" : isActive ? step.color : "rgba(255,255,255,0.15)",
                    transition: "color 0.3s",
                  }}>
                    {pct}%
                  </span>
                ...
                {/* Individual bar — only for active and completed */}
